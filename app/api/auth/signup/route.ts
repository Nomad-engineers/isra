...
const PAYLOAD_API_URL = process.env.PAYLOAD_API_URL || BASE_URL;
...
async function createUser(data: {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      console.log("Payload CMS user creation failed: ", errorData);

      // Handle specific Payload CMS error responses
      if (response.status === 400) {
        const errorMessage =
          errorData.message || errorData.errors?.[0]?.message || "";
